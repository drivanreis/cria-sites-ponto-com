# File: backend/src/schemas/employee_schema.py
...
class EmployeeRead(BaseModel):
    ...
    class Config:
        # Isso permite que o Pydantic mapeie automaticamente os campos do SQLAlchemy Model
        # para o Pydantic Schema. Essencial para rotas GET.
        from_attributes = True # Para Pydantic v2.x
        # orm_mode = True # Para Pydantic v1.x (se estiver usando uma versão mais antiga)
...
class EmployeeUpdate(BaseModel):
    # employee_name: str # <--- Este campo NÃO deve ser incluído aqui para impedir a edição
...
# --- Schema para Criação Interna (Opcional, para a Inicialização) ---
# Este schema pode ser usado para validar os dados na função de inicialização da aplicação,
