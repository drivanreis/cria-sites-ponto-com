...
# Testes de integração de usuários
docker compose exec backend pytest tests/integration/users/test_users_integration.py >> "$LOG_FILE" 2>&1
docker compose exec backend pytest tests/integration/users/test_users_permissions.py >> "$LOG_FILE" 2>&1
