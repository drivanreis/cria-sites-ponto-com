...
import type { BriefingWithHistory } from '../../api/briefings'; // ✅ Correção aqui
...
const BriefingDetailPage: React.FC = () => {
  ...
  return (
    <div className="App">
      ...
      {briefing.conversation_history && briefing.conversation_history.length > 0 ? (
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {briefing.conversation_history.map((msg, index) => (
            <li key={index} style={{ marginBottom: '10px' }}>
              <strong>{msg.sender_type}</strong> ({msg.timestamp}): {msg.message_content}
            </li>
          ))}
        </ul>
      ) : (
      ...
      <input
        type="text"
        placeholder="Nome do funcionário (ex: Entrevistador Pessoal)"
        value={employeeName}
